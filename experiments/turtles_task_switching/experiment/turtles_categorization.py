...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\turtles_experiment\\turtles_categorization.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    expInfo['category_A_label']=category_A_label
    expInfo['category_B_label']=category_B_label
    ...
    # --- Initialize components for Routine "randomize_standard" ---
    ...
    # --- Initialize components for Routine "delete_switch_randomization" ---
    ...
    for thisCategorization_instruction in categorization_instructions:
        ...
        while continueRoutine:
            ...
            if key_resp_8.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_8.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        while continueRoutine:
            ...
            if key_resp_9.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_9.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        while continueRoutine:
            ...
            if key_resp_10.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_10.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        while continueRoutine:
            ...
            if key_resp_11.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_11.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        while continueRoutine:
            ...
            if key_resp_12.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_12.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        while continueRoutine:
            ...
            if key_resp_13.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_13.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        while continueRoutine:
            ...
            if key_resp_14.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_14.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        routineTimer.reset()
    # completed 1.0 repeats of 'categorization_instructions'
    ...
    for thisTraining in training:
        ...
        while continueRoutine:
            ...
            if key_resp_training.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_training.getKeys(keyList=['f','j'], ignoreKeys=None, waitRelease=False)
    ...
    for thisSize_judgment_trial in size_judgment_trials:
        ...
        while continueRoutine:
            ...
            if key_resp_size_judgment.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_size_judgment.getKeys(keyList=['f','j'], ignoreKeys=None, waitRelease=False)
    ...
    for thisTransfer_trial in transfer_trials:
        ...
        for thisTransfer_standard_instructions_loop in transfer_standard_instructions_loop:
            ...
            while continueRoutine:
                ...
                if space_8.status == STARTED and not waitOnFlip:
                    theseKeys = space_8.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
            ...
            while continueRoutine:
                ...
                if space_10.status == STARTED and not waitOnFlip:
                    theseKeys = space_10.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
            ...
            transfer_standard_instructions_p3 = data.Routine(
                ...
                components=[transfer_standard_instructions_text_2, space_12],
            )
            ...
            while continueRoutine:
                ...
                if space_12.status == STARTED and not waitOnFlip:
                    theseKeys = space_12.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        # set up handler to look after randomisation of conditions etc
        transfer_standard_loop = data.TrialHandler2(
            name='transfer_standard_loop',
            nReps=num_repeats_1, 
            method='sequential', 
            extraInfo=expInfo, 
            originPath=-1, 
        ...
        thisExp.addLoop(transfer_standard_loop)  # add the loop to the experiment
        thisTransfer_standard_loop = transfer_standard_loop.trialList[0]  # so we can initialise stimuli with some values
        # abbreviate parameter names if possible (e.g. rgb = thisTransfer_standard_loop.rgb)
        if thisTransfer_standard_loop != None:
            for paramName in thisTransfer_standard_loop:
                globals()[paramName] = thisTransfer_standard_loop[paramName]
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        
        for thisTransfer_standard_loop in transfer_standard_loop:
            currentLoop = transfer_standard_loop
            thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
            if thisSession is not None:
                # if running in a Session with a Liaison client, send data up to now
                thisSession.sendExperimentData()
            ...
            # --- Prepare to start Routine "randomize_standard" ---
            # create an object to store info about Routine randomize_standard
            randomize_standard = data.Routine(
                name='randomize_standard',
                components=[],
            )
            randomize_standard.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from randomize_standard_code
            import pandas as pd
            s_standard=pd.read_csv("stim/stim_transfer_standard.csv")
            do_sample=True
            while do_sample:
                s_standard_shuffled=s_standard.sample(frac=1).reset_index(drop=True)
                X=0 
                # make sure first trial is not a switch
                if s_standard_shuffled.iloc[0]["trial_type"]=="switch":
                    X=X+1
                # make sure there are no consecutive switch trials
                for j in range(s_standard_shuffled.shape[0]-1):
                    if (s_standard_shuffled.iloc[j]["trial_type"]=='switch'):
                        if (s_standard_shuffled.iloc[j+1]["trial_type"]=='switch'):
                            X=X+1
                if(X==0):
                    do_sample=False
            s_standard_shuffled.to_csv("stim/stim_transfer_standard_random_tmp.csv",index=False)
            # store start times for randomize_standard
            randomize_standard.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            randomize_standard.tStart = globalClock.getTime(format='float')
            randomize_standard.status = STARTED
            thisExp.addData('randomize_standard.started', randomize_standard.tStart)
            randomize_standard.maxDuration = None
            # keep track of which components have finished
            randomize_standardComponents = randomize_standard.components
            for thisComponent in randomize_standard.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "randomize_standard" ---
            # if trial has changed, end Routine now
            if isinstance(transfer_standard_loop, data.TrialHandler2) and thisTransfer_standard_loop.thisN != transfer_standard_loop.thisTrial.thisN:
                continueRoutine = False
            randomize_standard.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine:
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
                ...
                # check if all components have finished
                if not continueRoutine:  # a component has requested a forced-end of Routine
                    randomize_standard.forceEnded = routineForceEnded = True
                    break
                continueRoutine = False  # will revert to True if at least one component still running
                for thisComponent in randomize_standard.components:
                    if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            ...
            # --- Ending Routine "randomize_standard" ---
            for thisComponent in randomize_standard.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for randomize_standard
            randomize_standard.tStop = globalClock.getTime(format='float')
            randomize_standard.tStopRefresh = tThisFlipGlobal
            thisExp.addData('randomize_standard.stopped', randomize_standard.tStop)
            # the Routine "randomize_standard" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            ...
            transfer_standard = data.TrialHandler2(
                ...
                extraInfo=expInfo, 
                originPath=-1, 
                trialList=data.importConditions('stim/stim_transfer_standard_random_tmp.csv'), 
                seed=None, 
            )
            ...
            for thisTransfer_standard in transfer_standard:
                ...
                while continueRoutine:
                    ...
                    if key_resp_transfer_standard.status == STARTED and not waitOnFlip:
                        theseKeys = key_resp_transfer_standard.getKeys(keyList=['f','j'], ignoreKeys=None, waitRelease=False)
            ...
            # completed 1.0 repeats of 'transfer_standard'
            
            if thisSession is not None:
                # if running in a Session with a Liaison client, send data up to now
                thisSession.sendExperimentData()
            
            # --- Prepare to start Routine "delete_switch_randomization" ---
            # create an object to store info about Routine delete_switch_randomization
            delete_switch_randomization = data.Routine(
                name='delete_switch_randomization',
                components=[],
            )
            delete_switch_randomization.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from delete
            os.remove("stim/stim_transfer_standard_random_tmp.csv")
            del do_sample
            # store start times for delete_switch_randomization
            delete_switch_randomization.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            delete_switch_randomization.tStart = globalClock.getTime(format='float')
            delete_switch_randomization.status = STARTED
            thisExp.addData('delete_switch_randomization.started', delete_switch_randomization.tStart)
            delete_switch_randomization.maxDuration = None
            # keep track of which components have finished
            delete_switch_randomizationComponents = delete_switch_randomization.components
            for thisComponent in delete_switch_randomization.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "delete_switch_randomization" ---
            # if trial has changed, end Routine now
            if isinstance(transfer_standard_loop, data.TrialHandler2) and thisTransfer_standard_loop.thisN != transfer_standard_loop.thisTrial.thisN:
                continueRoutine = False
            delete_switch_randomization.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine:
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
                ...
                # check if all components have finished
                if not continueRoutine:  # a component has requested a forced-end of Routine
                    delete_switch_randomization.forceEnded = routineForceEnded = True
                    break
                continueRoutine = False  # will revert to True if at least one component still running
                for thisComponent in delete_switch_randomization.components:
                    if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            ...
            # --- Ending Routine "delete_switch_randomization" ---
            for thisComponent in delete_switch_randomization.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for delete_switch_randomization
            delete_switch_randomization.tStop = globalClock.getTime(format='float')
            delete_switch_randomization.tStopRefresh = tThisFlipGlobal
            thisExp.addData('delete_switch_randomization.stopped', delete_switch_randomization.tStop)
            # the Routine "delete_switch_randomization" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            thisExp.nextEntry()
            
        # completed num_repeats_1 repeats of 'transfer_standard_loop'
        ...
        for thisTransfer_switch_instructions_loop in transfer_switch_instructions_loop:
            ...
            while continueRoutine:
                ...
                if space_14.status == STARTED and not waitOnFlip:
                    theseKeys = space_14.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
            ...
            while continueRoutine:
                ...
                if space_16.status == STARTED and not waitOnFlip:
                    theseKeys = space_16.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
            ...
            while continueRoutine:
                ...
                if space_18.status == STARTED and not waitOnFlip:
                    theseKeys = space_18.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
            ...
            while continueRoutine:
                ...
                if space_to_begin_switch.status == STARTED and not waitOnFlip:
                    theseKeys = space_to_begin_switch.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        for thisTransfer_switch_loop in transfer_switch_loop:
            ...
            import pandas as pd
            s_switch=pd.read_csv("stim/stim_transfer_switch.csv")
            do_sample=True
            while do_sample:
                s_switch_shuffled=s_switch.sample(frac=1).reset_index(drop=True)
                X=0 
                # make sure first trial is not a switch
                if s_switch_shuffled.iloc[0]["trial_type"]=="switch":
                    X=X+1
                # make sure there are no consecutive switch trials
                for j in range(s_switch_shuffled.shape[0]-1):
                    if (s_switch_shuffled.iloc[j]["trial_type"]=='switch'):
                        if (s_switch_shuffled.iloc[j+1]["trial_type"]=='switch'):
                            X=X+1
                if(X==0):
                    do_sample=False
            s_switch_shuffled.to_csv("stim/stim_transfer_switch_random_tmp.csv",index=False)
            ...
            for thisTransfer_switch in transfer_switch:
                ...
                while continueRoutine:
                    ...
                    if key_resp_transfer_switch.status == STARTED and not waitOnFlip:
                        theseKeys = key_resp_transfer_switch.getKeys(keyList=['f','j'], ignoreKeys=None, waitRelease=False)
    ...
    while continueRoutine:
        ...
        if space_to_begin_debrief.status == STARTED and not waitOnFlip:
            theseKeys = space_to_begin_debrief.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
    ...
    while continueRoutine:
        ...
        if space_to_end.status == STARTED and not waitOnFlip:
            theseKeys = space_to_end.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
