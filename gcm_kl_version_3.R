...
mds_cat <- here("mds_turtles_categories_25_11/mds_orig_turtles_cat.csv") %>%
  read_csv() 
mdir <- here("mds_turtles_gcm_kl_version_3")
dir_create(mdir)
...
ps <- path(mdir,glue("mds_turtles_kl_version_3_results_c={round(c_true,digits=3)}_gamma1={gamma_1_true}_gamma2={gamma_2_true}_w={w_true}_n_trial={n_trial}_inc={inc}"))
...
all_plot
ggsave(all_plot,filename=glue("{ps}.jpeg"),width=8,height=10)
ggsave(all_plot,filename=glue("{ps}.pdf"),width=8,height=10)
